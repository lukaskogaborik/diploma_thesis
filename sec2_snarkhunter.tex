...
In this chapter, we introduce methods for generating cubic graphs with desired minimal girth. A program called \texttt{minibaum} \cite{minibaum} is based on the idea of the Moore tree, while \texttt{snarkhunter} \cite{snarkhunter, snarkhunter_better} generates the graphs from a few elementary ones by adding edges. 
...
The natural question is, how can we easily determine an edge, which is not reducible. Let us check it case by case.
\begin{itemize}
    \item $G\sim e$ is disconnected. Then, since $G$ is connected, the removing of edge $e$ disconnects the graph $G$, and so $e$ must be a bridge in $G$. Conversely, if $e$ is a bridge in $G$, then the graph $G\sim e$ is clearly disconnected.
    \item $G\sim e$ has a loop $e'$ in a vertex $x$. Then, in $G$, at least one endvertex of $e=uv$ lie on $e'$. When exactly one endvertex of $e$ lie on $e'$ (WLOG $u$), then there are multiple edges between $u$ and $x$. Otherwise, there are multiple edges betweeen $u$ and $v$. Hence, the original graph $G$ cannot be simple, a contradiction.
    \item $G\sim e$ has multiple edges $e_1, e_2$ connecting $x$ and $y$. Then, in $G$, at least one endvertex of $e=uv$ lie on either $e_1$ or $e_2$.
    \begin{itemize}
        \item One endvertex of $e$ (WLOG $u$) lie on one of the mutliple edges (WLOG $e_1$). Then, $x$, $y$ and $u$ form a triangle in $G$. Conversely, if $e$ is an edge with exactly one vertex in a triangle in $G$, the graph $G\sim e$ obviously contains mutliple edges.
...
\begin{definition}\cite[p. 70]{snarkhunter}
    Cubic graph $G$ is called \emph{prime}, if it has no reducible edge.
\end{definition}
...
We introduce two cubic multipoles relevant for the characterisation prime graphs, depicted in Figure \ref{fig:diamond_lollipop}. A \emph{diamond} is a $2$-pole $D$ arising from $K_4$ by splitting its edge to two dangling edges. Since $K_4$ is vertex-transitive, the definition of diamond does not depend on the chosen edge, and therefore is uniquely determined. On the other hand, by subdividing any edge of $K_4$ with a vertex and connecting this vertex to a new vertex, we get a \emph{lollipop} $1$-pole $L$.
...
\begin{lemma}\emph{\cite[p. 70]{snarkhunter}}
    Cubic graph is prime if and only if each of its edges is either a bridge or contained in a diamond.
\end{lemma}

However, the prime graphs can be also characterised in an inductive way, which allows us to generate them from base cases.
...
The other way to get two isomophic graphs of order $n$ during the generation, is to obtain them by extensions from two non-isomorphic graphs of order $n-2$. To decide, which of those extensions should be accepted, we use a canonical labeling of vertices provided by \texttt{nauty} \cite{nauty}. For each graph $G$, we accept only the reduction of an edge, which is in a same edge orbit as the reducible edge with the lexicografically largest canonical vertex labels.

However, determining a canonical labeling of vertices is a computationally hard problem (since otherwise, graph isomorhpism would not be in $NP$), so we aim to avoid computing canonical labels as often as possible. This can be done by computing some local invariants for the edges, because when two edges have a different invariant, they can not be in a same edge orbit. The invariants used by the authors of \texttt{snarkhunter} with the corresponding criteria are
\begin{enumerate}
...
\section{Priority of reductions}

The idea mentioned in the previous section distinguishes edgepairs considering the same reduction operation. However, besides edge reductions, the algorithm uses prime graph reductions and so-called \emph{triangle reductions}.

Note that the edge extension applied to a pair of adjacent edges results in a triangle. However, this operation may be viewed also as the common vertex of those edges blown up to a triangle. Moreover, it can be shown that almost all triangles can be reduced simultaneously. The irreducible ones are the triangles contained in some diamond. Moreover, there are pairs of reducible triangles, which cannot be reduced simultaneously -- those contained in a diamond with one vertex blown up to a triangle (see Figure \ref{fig:diamond_plus_triangle}).
...
However, reducing any of triangles in Figure \ref{fig:diamond_plus_triangle} results in a diamond, so it is not dependent on its choice. Hence, triangle operation reduces as many triangles as possible, and then the result is deterministic, so we do not need to find the canonical reduction. Therefore, if there are reducible triangles, authors prefer the triangle reduction over the edge reduction. Moreover, for the general case, we can assume that edges in the edgepair are disjoint.

Back to the priority of reductions. The triangle reduction has the highest priority, and is followed by the edge reduction. Hence, prime graph reductions may be used only on prime graphs. Finally, the order of prime graphs are (d) $>$ (c) $>$ (b) $>$ (a).
...
Since from the computational results, the tripod reduction does not help much for girth $5$, the actual implementation of \texttt{snarkhunter} uses edge reductions for girths up to $5$ and $\mathcal H$ reductions for girths $6$ and $7$.

\section{Other features of snarkhunter}

\textbf{TODO snarks, modulo computation, cyclic edge connectivity}